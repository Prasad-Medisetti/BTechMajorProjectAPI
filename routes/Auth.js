...
router.patch("/change_password/:id", verify, async (req, res) => {
	...
	if (newPassword!==confirmNewPassword) return res.status(400).json({"error":'passwords not matched'})
	...
	User.findByIdAndUpdate(id, { password: hashedPassword}, {new: true}, (error, savedData) => {
		if (error) {
			console.log('find by id and update error ' ,error)
			res.send(404);
		} else {
